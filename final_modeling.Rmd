...
calculate_avgs <- function(rating_string) {
  ...
  return( list(base.avg = base.avg, recent.avg = recent.avg, last.rating = last.rating, useful=useful, country=country))
}
...
names(agg.ratings)[4:8] <- cbind("base.avg.rating", "recent.avg.rating", "last.rating", "useful", "country")
...
If the value of base.avg.rating moves one unit (e.g. from 1 to 2), the odds of moving from a 1-star last.rating to a higher rating is doubled. 
Similarily, if the value of recent.avg.rating moves one unit (e.g. from 1 to 2), the odds of moving from a 1-star last.rating to a higher rating is multiplied by 1.8.


## Influence of country and the number of reviews at the time of the last rating

Since there is only data for 2 German businesses, exclude them:
```{r}
...
# n=25, bar=3.357143, rar=3.5, r=2, useful=0, US
```
